...
class ICollectiveMailinglijst(Interface):
...
class IMailinglijstLocator(Interface):
    """Mailinglijst API
    """

    def initialize():
        """Do a postmonkey with the API so your connected to mailinglijst API"""

    def lists():
        """
        Return all available Mailinglijst lists.
        """

    def default_list_id():
        """Return the default list from the settings if it exists.
        Else return the first list available in your mailinglijst account
        """

    """def groups(list_id=None):
        Return all available Mailinglijst interest groups.

        @id: the list id to connect to. e.g. u'a1346945ab'. Not the web_id!

        http://apidocs.mailinglijst.com/api/rtfm/listinterestgroupings.func.php
