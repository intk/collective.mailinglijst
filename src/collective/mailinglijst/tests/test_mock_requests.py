...
from collective.mailinglijst.testing import DUMMY_API_KEY
from collective.mailinglijst.testing import MockRequestsException
from collective.mailinglijst.testing import TEST_DATA_DIR
import os
import unittest
...
class MockRequestsTests(unittest.TestCase):
    ...
    def test_locator_requests_error(self):
        ...
        from collective.mailinglijst.locator import requests
        self.assertRaises(MockRequestsException, requests.get)
        self.assertRaises(MockRequestsException, requests.get, '')
        self.assertRaises(MockRequestsException, requests.get,
                          'http://website.unavailable')
        url = 'https://api.mailinglijst.com/3.0/'
